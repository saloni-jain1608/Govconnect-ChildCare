...
import savePopUp from "@salesforce/apex/CLQ_OnsiteReviewFindingControllerLWC.savePopUp";

export default class OnsiteReviewFindingPopupLwc extends LightningElement {
...
@track popupresmod=[];
    @api show() {
        ...
        console.log("popupresult popupWrapperFindings1:",this.popupresult.popupWrapperFindings1);
        console.log("popupresult popupWrapperFindings1:",this.popupresult.popupWrapper1);
        if(this.popupresult.popupWrapperFindings1.length!=0){
        this.popupresult.popupWrapperFindings1.forEach(ele=>{
        ...
        if(this.popupresult.popupWrapper1.length!=0){
            this.popupresult.popupWrapper1.forEach(ele=>{
                this.popupresmod.push({'Id':ele.standardFindings.CLQ_Standard_Finding__c,'isSelected':ele.isSelected,'standardFindings':ele.standardFindings,'rating':ele.rating,'point':ele.point,'description':ele.description,'sfindingName':ele.sfindingName,'sMetOrNotMet':ele.sMetOrNotMet});
            })}
    ...
    handleSave(){
        ...
        savePopUp({selectedRule:this.selectedRule,InspId:this.inspid,popupWrapperList:this.popupresult})
        .then(data=>{
